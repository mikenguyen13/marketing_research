#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# scan-journals.R -- weekly sweep of top marketing journals via the Crossref API.
#
# Pulls newly published work from the journals the book draws on, drops anything
# already cited in the repo's .bib files, and writes a candidate list for review.
# Verification is the point: every row carries a real Crossref-resolved DOI, so
# nothing downstream has to invent one.
#
# Usage:  Rscript .editorial/weekly/scan-journals.R [days_back]
# Output: .editorial/weekly/candidates-<today>.tsv  (one row per candidate)
#         .editorial/weekly/candidates-<today>.md   (skimmable version)
# ---------------------------------------------------------------------------
...
days_back <- if (length(args) >= 1) as.integer(args[1]) else 10L
mailto    <- "nguyennghia1301@gmail.com"   # polite-pool identifier for Crossref
...
cutoff <- format(Sys.Date() - days_back, "%Y-%m-%d")
today  <- format(Sys.Date(), "%Y-%m-%d")

# --- journals ---------------------------------------------------------------
# `broad = TRUE` marks general-interest journals where only a slice of each
# issue is marketing; those get keyword-screened below instead of taken whole.
journals <- rbind(
...
# Screen for the broad journals. A single keyword list does badly here: finance
# and OM papers say "pricing", "rating", "platform" constantly. So terms are split
# into strong (marketing-specific -- one hit is enough) and weak (generic -- needs
# corroboration). Tuned against a live Management Science pull: 20/51 kept, with
...
# Crossref ships JATS XML in BOTH abstracts and titles (Wiley titles routinely
# carry <scp>...</scp> small-caps markup, and some fields contain raw newlines
# and tabs). Left alone, those newlines split a record across several lines of
# the TSV and corrupt everything downstream -- so squish here, not later.
clean_txt <- function(x, drop_lead_abstract = FALSE) {
  if (is.null(x) || length(x) == 0 || is.na(x)) return(NA_character_)
  x <- gsub("<[^>]+>", " ", x)                       # JATS / HTML tags
  x <- gsub("&[a-zA-Z]+;|&#[0-9]+;", " ", x)         # entities
  if (drop_lead_abstract) x <- gsub("^\\s*Abstract\\s*", "", x)
  x <- gsub("[\r\n\t]+", " ", x)                     # newlines/tabs break the TSV
  x <- gsub("\\s+", " ", x)
  x <- trimws(x)
...
fetch_journal <- function(issn, name, cutoff, mailto) {
  url <- sprintf(
    paste0("https://api.crossref.org/journals/%s/works",
           "?filter=from-pub-date:%s,type:journal-article",
  ...
  message(sprintf("  ok %s: %d", name, nrow(df)))
  df
}

# --- fetch ------------------------------------------------------------------
message(sprintf("Crossref sweep: published since %s (%d days back)\n", cutoff, days_back))

all_rows <- list()
for (i in seq_len(nrow(journals))) {
  j  <- journals[i, ]
  df <- fetch_journal(j$issn, j$name, cutoff, mailto)
  ...
  Sys.sleep(0.4)   # stay friendly to the API
}
...
for (f in bib_files) {
  ...
  m   <- regmatches(txt, gregexpr("(?i)doi\\s*=\\s*[{\"]\\s*([^}\"]+)", txt, perl = TRUE))
  m   <- unlist(m)
  if (length(m)) {
    d <- sub("(?i)^doi\\s*=\\s*[{\"]\\s*", "", m, perl = TRUE)
    known_doi <- c(known_doi, tolower(trimws(d)))
  }
}
# DOIs pasted directly into chapter prose (replication boxes, reading lists) count too.
...
# --- drop anything a previous week already surfaced -------------------------
# The scan window overlaps week to week on purpose (so a missed run loses
# nothing), which means the same paper would otherwise reappear every Monday.
# Pass --all to ignore the ledger and re-report everything in the window.
ledger  <- file.path(out_dir, "seen-dois.txt")
use_all <- "--all" %in% args
seen    <- if (file.exists(ledger) && !use_all) {
  tolower(trimws(readLines(ledger, warn = FALSE)))
} else character(0)
seen <- seen[nzchar(seen)]

n_seen <- sum(tolower(cands$doi) %in% seen)
fresh  <- cands[!(tolower(cands$doi) %in% seen), , drop = FALSE]

# Backlog = surfaced in an earlier week, still not cited anywhere in the book.
backlog <- setdiff(seen, known_doi)

if (nrow(fresh) > 1) {
  yr <- suppressWarnings(as.integer(fresh$year))
  yr[is.na(yr)] <- 0L
  fresh <- fresh[order(fresh$journal, -yr), , drop = FALSE]
}
cands <- fresh

if (!use_all && nrow(cands) > 0) {
  cat(tolower(cands$doi), file = ledger, sep = "\n",
      append = file.exists(ledger))
  cat("\n", file = ledger, append = TRUE)
}

message(sprintf("\n%d new candidates (%d already cited, %d seen in earlier weeks)",
                nrow(cands), n_dup, n_seen))
message(sprintf("Repo has %d known DOIs; backlog (surfaced before, still uncited): %d",
                length(known_doi), length(backlog)))

# --- write ------------------------------------------------------------------
tsv <- file.path(out_dir, sprintf("candidates-%s.tsv", today))
write.table(cands, tsv, sep = "\t", row.names = FALSE, quote = TRUE, fileEncoding = "UTF-8")

md   <- file.path(out_dir, sprintf("candidates-%s.md", today))
con  <- file(md, open = "w", encoding = "UTF-8")
writeLines(c(
  sprintf("# Crossref candidates -- %s", today),
  "",
  sprintf("Published since **%s** (%d days back). %d new candidates after removing %d already cited and %d surfaced in earlier weeks.",
          cutoff, days_back, nrow(cands), n_dup, n_seen),
  sprintf("Backlog (surfaced in an earlier week, still not cited anywhere in the book): **%d**.",
          length(backlog)),
  "",
